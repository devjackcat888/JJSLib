...
public class JJSTabBar: UITabBar {

    public private(set) var buttons: [JJSTabBarButton] = []
    ...
    func reloadViews() {
        ...
        buttons = items?.map({ (item) -> JJSTabBarButton in
            return self.button(forItem: item)
        }) ?? []

        var lastButton: JJSTabBarButton?
    ...
    private func button(forItem item: UITabBarItem) -> JJSTabBarButton {
        let button = JJSTabBarButton(item: item)
    ...
    @objc private func btnPressed(sender: JJSTabBarButton) {
